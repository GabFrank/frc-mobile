...
- Secretos, `.env`, keystores, certificados, `google-services.json`
- `capacitor.config.ts` sin coordinar (requiere release nativo a Play Store)
- Metodos backend existentes usados por desktop — crear `*Mobile()` paralelo en su lugar
- Archivos en `android/` sin correr `npx cap sync android` despues
