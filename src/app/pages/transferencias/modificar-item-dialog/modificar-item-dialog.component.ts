...
export class ModificarItemDialogComponent implements OnInit {
  ...
  async ngOnInit() {
    if (this.data?.item?.presentacionPreTransferencia != null) {
      (
        await this.presentacionService.onGetPresentacionesPorProductoId(
          this.data.item.presentacionPreTransferencia?.producto?.id
        )
      ).subscribe((presentacionRes) => {
    ...
    switch (this.data.transferencia.etapa) {
      case EtapaTransferencia.PREPARACION_MERCADERIA:
        this.cantidadAnterior =
          this.data.item?.cantidadPreparacion ||
          this.data.item.cantidadPreTransferencia;
        this.vencimientoAnterior =
          this.data.item?.vencimientoPreparacion ||
          this.data.item.vencimientoPreTransferencia;
        this.presentacionAnterior =
          this.data.item?.presentacionPreparacion ||
          this.data.item.presentacionPreTransferencia;
        break;
      case EtapaTransferencia.TRANSPORTE_VERIFICACION:
        this.cantidadAnterior =
          this.data.item?.cantidadTransporte ||
          this.data.item.cantidadPreparacion;
        this.vencimientoAnterior =
          this.data.item?.vencimientoTransporte ||
          this.data.item.vencimientoPreparacion;
        this.presentacionAnterior =
          this.data.item?.presentacionTransporte ||
          this.data.item.presentacionPreparacion;
        break;
      case EtapaTransferencia.RECEPCION_EN_VERIFICACION:
        this.cantidadAnterior =
          this.data.item?.cantidadRecepcion || this.data.item.cantidadTransporte;
        this.vencimientoAnterior =
          this.data.item?.vencimientoRecepcion ||
          this.data.item.vencimientoTransporte;
        this.presentacionAnterior =
          this.data.item?.presentacionRecepcion ||
          this.data.item.presentacionTransporte;
        break;
      default:
        break;
  ...
  onAceptar() {
    if (this.cantidadControl.value == null)
      this.cantidadControl.setValue(this.cantidadAnterior);
    if (this.vencimientoControl.value == null)
      this.vencimientoControl.setValue(this.vencimientoAnterior);
    if (this.presentacionControl.value == null)
      this.presentacionControl.setValue(this.presentacionAnterior);
